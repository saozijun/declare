...
//        }
        ...
        if(one!=null){
            // 判断用户状态
            if (one.getStatus() == 0) {
                throw new ServiceException(Constants.CODE_402,"账号审核中");
            }
    ...
    @Override
    public Result getAvailableUsersForExpert() {
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("role", "user");
        queryWrapper.eq("status", 1); // Only active users
    ...
    public UserDTO userLogin(UserDTO userDTO) {
        ...
        if (one != null) {
            if (one.getRoleId() != 2) {
                throw new ServiceException(Constants.CODE_402, "该账号不是用户账号");
            }
            // 判断用户状态
            if (one.getStatus() == 0) {
                throw new ServiceException(Constants.CODE_402, "账号审核中");
            ...
            throw new ServiceException(Constants.CODE_402, "用户名或密码错误，或用户角色不正确");
        }
    }

    // 专家注册用户逻辑
    @Override
    public Long expertRegister(UserDTO userDTO) {
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("username", userDTO.getUsername());
        User one = getOne(queryWrapper);
        if (one == null) {
            ...
            one.setRole("expert");
            one.setRoleId(3);
            one.setStatus(0); // 设置为审核中状态
            save(one);
            return one.getId(); // 返回新创建的用户ID
        } else {
            throw new ServiceException(Constants.CODE_500, "用户已存在");
        }
    }
